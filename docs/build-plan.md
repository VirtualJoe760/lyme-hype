...
- [x] **Wire Generate to a real connector — done, agent-driven.** `src/main/generation.ts` runs the Claude Agent SDK with the installed **stdio** connectors attached as `mcpServers` (each credential injected into its env exactly as a real call would), and lets the agent pick the right generation tool for the requested media type — no per-connector arg-schema hardcoding, which matters while connectors "aren't finished being built." It returns a strict `RESULT_URL:`/`RESULT_ERROR:` line; the URL is imported via `importUrlAsset` (now Content-Type/media-type aware) and dropped as a real node. The aside's Generate button and Storyboard **promote** (when the panel has a note) both drive it through `store.generateMedia`, with a real "Rendering…" → ready **or error** lifecycle (nodes gained an `error` status). **Safety:** no `bypassPermissions` — `allowedTools` pre-authorizes only the `mcp__<server>` tools and a `canUseTool` backstop hard-denies every non-MCP tool, so the on-machine agent can never run Bash/Write/etc.; orchestration is capped by `maxBudgetUsd` and a 5-min timeout. **Boundary:** the live generation call itself is deliberately untested here (deferred to a session with the user); verified up to that edge — renderer lifecycle (create → await → ready/error) exercised in the browser mock, the no-connector and empty-prompt error paths, typecheck + build, and a self-test case that only trips the pre-connector guard.
- [x] **http-MCP client** — done. `src/main/mcp-http.ts` speaks the Streamable-HTTP MCP transport (POST JSON-RPC, parses JSON *or* `text/event-stream`, carries the `Mcp-Session-Id`). `testConnector` now probes http connectors as well as stdio, and generation's `buildMcpServers` attaches http connectors to the agent as `{type:'http',url,headers}` (the Agent SDK carries the real transport at generation time). Krea (`api.krea.ai/mcp`) and fal (`mcp.fal.ai/mcp`) are flipped to available with http templates (bearer `Authorization`); the credential injects as a header via `httpAuthHeaders`. fal may want `Authorization: Key <k>` — the note says to adjust if Test 401s. Live vendor calls untested here (deferred), but the probe was exercised against a public reference MCP endpoint.
- [x] **MCP OAuth** — done. `src/main/mcp-oauth.ts`: protected-resource discovery → auth-server metadata → dynamic client registration → PKCE in the system browser → loopback redirect → token exchange; tokens live in the vault under the connector id and refresh silently. Yapper installs from the catalog as `authType: 'oauth'`; the card shows **Connect account** instead of a secret prompt, and the custom-connector form offers oauth for http connectors. Same credential boundary as ever — nobody types anything, the agent never sees a token.
- [x] **Gemini stdio wrapper** — done, dependency-free. `resources/gemini-mcp.cjs` is a plain-Node stdio MCP server hitting the REST API directly (`GEMINI_API_KEY` env from the vault): `gemini_generate_image` (Nano Banana) + `gemini_generate_video` (Veo, polls the long-running op, downloads with the key in-process). Results hand off as `RESULT_FILE: <path>` — Gemini returns bytes/authed URIs, not public URLs — and generation now accepts `RESULT_FILE:` alongside `RESULT_URL:` (imports via `importFileAsset`). Protocol smoke-tested: initialize + tools/list + keyless call error all correct.
...
- **ffmpeg for distribution:** bundle a verified LGPL build (check `ffmpeg -version` has no `--enable-gpl`/`--enable-nonfree`) via `extraResources`, and switch `buildConcatArgs` from `libx264` → `openh264`. Local/personal use needs none of this (AGENTS.md §7).
