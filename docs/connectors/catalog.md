...
- **Training routes through fal now** (`src/main/fal-training.ts`, user call 2026-08-09): fal hosts both `fal-ai/krea-2-trainer` ($0.003/step, outputs `lora_file` safetensors) and `fal-ai/flux-krea-trainer` (FLUX.1 Krea [dev], ~$2/run, outputs `diffusers_lora_file`) with published pricing on the already-installed fal key — training images are packed into a store-only ZIP locally (dependency-free writer, verified against Python's zipfile), uploaded via fal storage, then queue-submitted and polled. The Krea-native `/styles/train` client this replaced lives in git history; the facts below about Krea's own API remain true if that route ever returns.
