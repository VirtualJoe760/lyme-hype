...
- **Key page:** `krea.ai/settings/api-tokens` · **Connect (general image/video/3D):** http MCP `api.krea.ai/mcp`, bearer `Authorization` (endpoint + Bearer/OAuth both live-probed 2026-08-09) · **Status: general connector built and installable; training is REST-only and reached by the dedicated client below.**
