...
|---|---|---|---|
| **Generate video** | prompt + aspect/duration/res (+ connector override, optional canvas-image starting frame, optional Yapper model pick); separately, pick an existing ready video node + a short "what happens next" prompt | video node | `video-gen-t2v` (any provider of it — muapi default when present); `video-gen-i2v` via a starting-frame image node (Gemini/Veo only — forces that connector, since it's the only wired i2v path today); video-extension (`gemini_extend_video`, forces Gemini, fixed +7s per call) via the "Extend an existing clip" picker — the node's `videoDurationSec` (probed client-side off the actual media element after every generation/extension, not the requested duration) drives the run-line's remaining-budget display and client-side-blocks a call that would exceed Veo's 148s chained-extension cap; the wrapper itself also enforces the cap server-side when it's told the running total |
