...
**Not built yet.** Replaces `src/renderer/src/components/AsidePanel.tsx`'s current flat form — media tabs, mode tabs, one prompt box, dimension chips, Generate/Upload/Link buttons, and an agent-link test card all stacked in one scroll — with a two-level, task-first UI. The current panel reads as "functionality and testing slapped together" because it is one: every control for every job lives on screen at once, all the time, regardless of what the user is actually trying to do.
