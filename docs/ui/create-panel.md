...
- **Krea training is `src/main/krea-training.ts`** — multipart POST to the documented `/styles/train` + poll loop, accepting any `{status}`-shaped reply and surfacing every non-2xx body verbatim, because the exact schema hasn't been exercised against a live token yet (that's joint-session live-spend work, same class as generation calls). Trained styles persist to `trained-styles.json` and show in **Settings › Trained styles**, per the firm decision below.
- **A trained style in the Image tile currently routes via `connectorId: 'krea'` + a model hint naming the style** — whether Krea's MCP generation surface can actually reference a trained style by name is untestable without a live token; flagged for the joint session rather than assumed.
