...
- ~~Agent auth for distribution~~ **Resolved (2026-08-08): Lyme Hype is a personal tool, not a distributed product, so this isn't a distribution question.** The agent authenticates with **this machine's own Claude Code login by default** — the same subscription-backed credential `claude setup-token` mints, and the same mechanism jboogx's Night Shift and openclaw (pre-spoofing-crackdown) run on. `agent.ts` passes no env override, so the SDK resolves auth however it already does on this machine; nothing needs to be configured for the normal case. `src/main/claude-auth.ts` additionally supports an *explicit* override — a dedicated `CLAUDE_CODE_OAUTH_TOKEN` (from `claude setup-token`, for a machine with no local Claude Code login) or a separate `ANTHROPIC_API_KEY` — entered via the native secure modal → `safeStorage`, same as any other credential. Anthropic's restriction on third-party apps *offering* claude.ai login is about multi-tenant products where other people log in through your app; it doesn't describe a solo developer running their own tool on their own subscription.
